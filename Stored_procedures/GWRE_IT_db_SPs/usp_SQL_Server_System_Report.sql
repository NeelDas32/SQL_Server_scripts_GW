...
/****** Object:  StoredProcedure [dbo].[usp_SQL_Server_System_Report]    Script Date: 7/30/2024 11:17:40 AM ******/
SET ANSI_NULLS ON
GO
...
SET @vRecipients = ''
SET @vRecipients = 'cchappidi@guidewire.com'      
...
SET @vSubject = 'SQL Server System Report: '+'ICAREPREPROD'    
SET @vXML_String = ''      
SET @vBody = ''      
...
--set @AVGCPU= (select AVG(convert(numeric(8,2),LogValue)) from [CHOLA-MCAS].CPUGRAPH.dbo.PerfLogs_55_live  
--where DAY(LogTime)=day(GETDATE()-1)  
--and DATEPART(HOUR,logtime) between 10 and 19
--)     
SELECT      
...
FROM      
 ...
 LEFT JOIN      
  ...
  (      
   SELECT      
     SJS.job_id      
    ,MIN (CONVERT (DATETIME, CONVERT (VARCHAR (8), SJS.next_run_date)+' '+LEFT (RIGHT ('000000'+CONVERT (VARCHAR (6), SJS.next_run_time),6),2)+':'+SUBSTRING (RIGHT ('000000'+CONVERT (VARCHAR (6), SJS.next_run_time),6),3,2)+':'+RIGHT (RIGHT ('000000'+CONVERT (VARCHAR (6), SJS.next_run_time),6),2))) AS next_run_date_time  FROM  msdb.dbo.sysjobschedules SJS WHERE  SJS.next_run_date > 0      
   GROUP BY      
    SJS.job_id      
...
@recipients = 'cchappidi@guidewire.com'      
 ,@Copy_Recipients =''
,@blind_copy_recipients ='cchappidi@guidewire.com'
  ,@subject = @vSubject      
