...
/****** Object:  StoredProcedure [dbo].[gwre_BackupDBLogs]    Script Date: 04/5/2018 14:26:41 ******/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[gwre_BackupDBLogs]') AND type in (N'P', N'PC'))
DROP PROCEDURE [dbo].[gwre_BackupDBLogs]
GO

USE [GWRE_IT]
GO

/****** Object:  StoredProcedure [dbo].[gwre_BackupDBLogs]    Script Date: 01/07/2015 14:14:11 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[gwre_BackupDBLogs] 
...
SET @DateSerial = CONVERT(NVARCHAR, GETDATE(), 112) + '_' 
...
WHERE state = 0 AND --online 
      ...
      name NOT IN ('master', 'tempdb', 'msdb') AND
...
/* Added an underscore between DBName and Date stamp - srk 05/2012 */
    SET @LogBackup = 
        @LogBackup + @DBName + '_' + @DateSerial + '.' + @FileExtension 

/* Added Compression to SQL command - srk 05/2012 */
    SET @SQL = 'Backup Log ' + QUOTENAME(@DBName) + CHAR(10) + CHAR(9) + 
			'To Disk = ''' + @LogBackup + '''' + CHAR(10) + CHAR(9) + 
